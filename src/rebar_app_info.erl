...
app_file_src(#app_info_t{app_file_src=undefined, dir=Dir, name=Name, opts=_Opts}) ->
    %% hexpm-cli: Use the generated .app instead.
    CandidatePaths = [filename:join([rebar_utils:to_list(Dir), Ebin, rebar_utils:to_list(Name)++".app"])
                      || Ebin <- ["ebin"]],
